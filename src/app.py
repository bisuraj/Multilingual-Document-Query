...
BASE_DIR = "E:\\Projects\\NLP_\\Final\\tr"
...
from pdfHandler import translateToEnglish
...
def _load_file(file):
    ...
    if os.path.isfile(BASE_DIR + f"/{base}_tr{ext}"):
        # print(BASE_DIR + f"/{base}_tr{ext}")
        
        file = open(BASE_DIR + f"/{base}_tr{ext}", "rb")
        return file
    return file
...
def ui_info():
    st.markdown(f"""
    ## InterLingo Document Query
    version {__version__}
    
    Multilingual PDF Query System Based on LLM Models
    """)
    ui_spacer(1)
    ui_spacer(1)
    st.markdown("""
    ...
    if ss['pdf_file']:
        ...
        if ss['filename'] != ss.get('fielname_done'): # UGLY
            ...
            with st.spinner(f'indexing {ss["filename"]}'):
                ...
                # save and and extract text in the file
                file = _load_file(ss['pdf_file'])
                                    
                if file == ss['pdf_file']:
                    translateToEnglish(ss['pdf_file'])
...
def ui_model():
    ...
    st.selectbox('main model', models, key='model', disabled=False)
...
def b_ask():
    ...
    if c2.button('👍', use_container_width=True, disabled=disabled):
        ss['feedback'].send(+1, ss, details=ss['send_details'])
        ss['feedback_score'] = ss['feedback'].get_score()
    if c3.button('👎', use_container_width=True, disabled=disabled):
        ss['feedback'].send(-1, ss, details=ss['send_details'])
        ss['feedback_score'] = ss['feedback'].get_score()
